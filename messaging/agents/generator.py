...
_SLIDE_INSTRUCTIONS: dict[str, str] = {
    ...
    "ai_winner_loser": (
        "这是全片最关键的一页，必须客观呈现AI赢家vs输家的两面论证：\n"
        "- 首先用1句话提出核心争议：'市场对这家公司的AI叙事存在根本性分歧'\n"
        "- 【AI赢家论据】列出2-3个具体的看多理由（必须有数据支撑）\n"
        "- 【AI输家论据】列出2-3个具体的看空理由（必须有数据支撑，不能比看多更软）\n"
        "- 结尾提出问题，不给答案：'市场最终会站哪边？下面我们逐个验证'\n"
        "禁止：偏向任何一方，禁止在本段给出结论，禁止说'我认为'"
    ),
    "bull_case": (
        "专属看多页面，必须写得像真正的多头最强辩护状：\n"
        "第一点：最硬核的看多数据（估值错配/EPS超预期/增长加速），必须有具体数字\n"
        "第二点：护城河或竞争优势，说明为什么这家不会被竞争对手取代\n"
        "第三点：催化剂，下一个改变定价的具体事件是什么、时间节点\n"
        "每个要点：结论（1句）→ 数据（1-2句）→ 类比（1句）→ 投资含义（1句）\n"
        "结尾一句金句式总结：看多逻辑的核心主张"
    ),
    "bear_case": (
        "专属看空页面，权重与bull_case完全对等，必须写得和看多一样有力：\n"
        "第一点：最致命的估值/业务风险，必须有具体数字（不能是模糊描述）\n"
        "第二点：竞争威胁或颠覆风险，说明哪个竞争对手或趋势在侵蚀护城河\n"
        "第三点：最可能的下行催化剂，时间节点和触发条件\n"
        "每个要点：结论（1句）→ 数据（1-2句）→ 机制（1句）→ 影响量级（1句）\n"
        "结尾：'知道了风险，才能管好仓位' — 不劝退，而是明确风险管理思路\n"
        "禁止：比bull_case更软，必须是真正的最强看空论据"
    ),
    "market_overview": (
        "- 从宏观或行业背景切入，1-2句定性\n"
    ...
    "price_action": (
        ...
        "- 支撑位：$X（来源：MA20/布林下轨等）\n"
        "- 阻力位：$X（来源：MA50/布林上轨等）\n"
        "- RSI：数值 + 信号（neutral/overbought/oversold）\n"
        "- 结论（1句）：现在是追还是等？\n"
        "禁止超过300字。不要解释每个指标是什么。直接给数字和判断。"
    ),
    "key_points": (
        "注意：key_points已被拆分为bull_case和bear_case，此指令仅作备用。\n"
        "如果此幻灯片出现，请以50%看多/50%看空的平等权重展开，不得偏向任何一方。"
    ),
    "news": (
        "新闻影响分析（如单独出现）：\n"
    ...
    "catalyst": (
        "催化剂 + 新闻影响分析合并页：\n"
        "【近期重要新闻】逐条分析：事件 → Bullish/Bearish → High/Medium/Low → 1句解读\n"
        "【正向催化剂】时间节点 + 潜在影响量级 + 触发条件\n"
        "【负向催化剂】时间节点 + 潜在影响量级 + 触发条件\n"
...
class GeneratorAgent(BaseAgent):
    ...
    def _smooth_narration(self, plan: dict, narration: str) -> str:
        ...
        system = (
            "你是资深财经视频脚本编辑，有两个任务：整合连贯性 + 去除AI感。"
            "不改变任何数据和分析结论，只改语言表达。"
        )
        user_msg = (
            f"以下是{ticker}（{company}）股票点评视频的分段口播稿，今天是{current_date}，总共{segment_count}段。\n\n"
            "**任务一：连贯性修复**\n"
            "1. 非首段出现问候语（大家好、朋友们等）→ 直接删除\n"
            "2. 中间段重新介绍日期、股票名称 → 删除重复\n"
            "3. 段落之间缺衔接 → 加一句自然过渡\n\n"
            "**任务二：去除AI感（关键）**\n"
            "让人听不出是AI写的，具体改法：\n"
            "1. 打破重复句式：如果相邻两段都用「第一点...第二点...第三点」结构，至少改掉一段的结构\n"
            "2. 多样化类比：如果出现3个以上「这就像...」，改掉其中2个，换成其他表达方式\n"
            "3. 加入节奏变化：在每段的关键结论后，偶尔插入一个只有1-2句的短段落，制造停顿感\n"
            "4. 口语化过渡：把「接下来」「综上所述」「综合来看」换成更自然的：「说到这里」「等一下」「换个角度」\n"
            "5. 允许不完美：偶尔用「说实话」「不瞒你说」「这个问题不好回答」等真实说话感\n\n"
            "**修改边界（绝对不能改）：**\n"
            "- 所有价格、数字、百分比、日期\n"
            "- 分析结论和投资判断\n"
            "- [幻灯片: xxx] 标注位置\n"
        ...
        print("[Generator] Editor pass: smoothing + naturalizing...")
        result = self._chat(system, user_msg, model=MODEL_CHAT)
    ...
    def generate_narration(self, plan: dict, synthesis: str) -> str:
        ...
        raw = "\n\n".join(parts)
        smoothed = self._smooth_narration(plan, raw)
        # Guard: if smooth pass returned < 60% of raw, it was likely truncated — use raw
        if len(smoothed) < len(raw) * 0.6:
            print(f"[Generator] Smooth pass truncated ({len(smoothed)}字 < 60% of {len(raw)}字), using raw.")
            full = raw
        else:
            full = smoothed
    ...
    _SLIDE_SCHEMAS: dict[str, str] = {
        ...
        "ai_winner_loser": (
            ...
            '"headline":"核心争议（10字以内）",'
            '"bull_points":["AI赢家论据1（含数字）","AI赢家论据2","AI赢家论据3"],'
            '"bear_points":["AI输家论据1（含数字）","AI输家论据2","AI输家论据3"],'
            '"open_question":"留给观众的核心问题（20字以内，不给结论）"}'
        ),
        "bull_case": (
            '{"type":"bull_case",'
            '"headline":"看多核心主张（10字以内）",'
            '"points":['
            '{"title":"要点标题（10字以内）","body":"展开说明含具体数字（50字以内）","tag":"bull"}'
            '],'
            '"verdict":"看多金句（20字以内，有记忆点）"}'
        ),
        "bear_case": (
            '{"type":"bear_case",'
            '"headline":"看空核心主张（10字以内）",'
            '"points":['
            '{"title":"要点标题（10字以内）","body":"展开说明含具体数字（50字以内）","tag":"bear"}'
            '],'
            '"verdict":"风险核心结论（20字以内）"}'
        ),
        "market_overview": (
